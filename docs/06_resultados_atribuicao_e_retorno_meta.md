...
> **Estado em 09/09/2026: R3, R4 e R6 CONSTRUÍDOS, com o envio desligado.** Decisão do dono: a D6 (LGPD) ficou para depois, então o sistema **registra** conversões desde já (`conversion_event`, gatilho em `contact.funnel_stage`, uma conversão por contato por etapa como no Tintim) e **não envia nada** até (a) credenciais na aba "Anúncios da Meta" e (b) a escolha do modo de dados pessoais (`meta_ads_account.modo_user_data`: só `ctwa_clid`, ou telefone hasheado com consentimento). A liberação, quando a D6 sair, tem DOIS passos: a **migration de liberação** (substitui a função `conferir_token_antes_de_ligar_envio`, que hoje embarga qualquer `modo_user_data` no banco, até para service role; migration 20260910150000) e virar a constante `DECISAO_LGPD_TOMADA` na aba. Aí o dono escolhe o modo, salva e ativa; os registros de até 7 dias entram na fila e os mais antigos são descartados com código (janela da CAPI). Detalhe confirmado na doc da Meta: evento de anúncio CTWA exige `action_source business_messaging` + `whatsapp_business_account_id` no `user_data`, então a aba pede o WABA; sem ele o evento cai no formato genérico por telefone.
