...
- `comum.tsx`: `SemDado` continua o hífen visual com `sr-only`; `BarraComparecimento` e `BarraSessoes` usam `BarraDeProgresso tom="neutro"` (`tamanho` sm na lista, md na ficha); `BlocoFicha` vira Card com cabeçalho (continua `section` + `h2`, o e2e procura); `LinhaDaFicha` pela receita 4.7.
